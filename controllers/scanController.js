...
exports.uploadScan = async (req, res) => {
  ...
  try {
    ...
    // 🧠 Save the image locally in Node.js backend
    fs.writeFileSync(savePath, req.file.buffer);

    // 🧠 Optional: Save in AI project (Flask side)
    // e.g., Copy to shared folder between Flask & Node.js
    const flaskPath = path.join('E:/AI_Cancer/Khaled_CancerChat/Khaled_CancerChat/static', filename);
...
exports.exportScanToPDF = async (req, res) => {
  try {
    ...
    // لازم تعيين رؤوس الاستجابة قبل عمل pipe
    ...
    if (scan.image_path) {
      const imagePath = path.join(__dirname, '../uploads', scan.image_path);
      ...
      doc
        .fontSize(14)
        .fillColor('red')
        .text('⚠️ No image was uploaded for this scan.', { align: 'center' });
    }

    // نهاية كتابة الملف وانتظار إنه يخلص قبل الرد النهائي
